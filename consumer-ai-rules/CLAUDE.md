# Flysoft React UI - Reglas para AI
...
Para layout completo con navbar, sidebar y snackbars:
...
| Prop | Tipo | Default | Descripción |
|------|------|---------|-------------|
| `gap` | `string` | — | Espaciado entre elementos |
| `direction` | `"row" \| "column"` | `"row"` | Dirección del flex |
| `wrap` | `boolean` | `false` | Permitir wrap |

### DataField

Par label + valor para mostrar datos.

| Prop | Tipo | Descripción |
|------|------|-------------|
| `label` | `string` | Etiqueta |
| `value` | `ReactNode` | Valor a mostrar |
| `inline` | `boolean` | Layout horizontal |
| `align` | `"left" \| "right" \| "center"` | Alineación |
| `link` | `string` | Convierte el valor en enlace |

### Menu\<T\>

Lista de opciones simple.

| Prop | Tipo | Descripción |
|------|------|-------------|
| `options` | `T[]` | **Requerido**. Opciones |
| `onOptionSelected` | `(option: T) => void` | Callback al seleccionar |
| `getOptionLabel` | `(item: T) => string` | Extraer texto |
| `renderOption` | `(item: T) => ReactNode` | Render personalizado |

### DropdownMenu\<T\>

Dropdown portal con opciones.

| Prop | Tipo | Descripción |
|------|------|-------------|
| `options` | `T[]` | **Requerido**. Opciones |
| `onOptionSelected` | `(option: T) => void` | Callback al seleccionar |
| `renderNode` | `ReactNode` | Elemento trigger |
| `openOnHover` | `boolean` | Abrir al hover |
| `replaceOnSingleOption` | `boolean` | Reemplazar trigger si hay una sola opción |

### DropdownPanel

Dropdown portal con contenido arbitrario.
...
| Prop | Tipo | Descripción |
|------|------|-------------|
| `isLoading` | `boolean` | Controla visibilidad |
| `text` | `string` | Texto debajo del spinner |
| `keepContentWhileLoading` | `boolean` | Mantener contenido visible |
| `contentLoadingNode` | `ReactNode` | Nodo personalizado durante carga |

### Skeleton

Placeholder con animación pulse para loading states.

| Prop | Tipo | Descripción |
|------|------|-------------|
| `className` | `string` | Clases CSS para dimensiones |
...
Notificaciones toast. Usar con `SnackbarProvider`.

| Prop (SnackbarContainer) | Tipo | Default | Descripción |
|------|------|---------|-------------|
| `position` | `"top-left"\|"top-right"\|"bottom-left"\|"bottom-right"` | `"bottom-right"` | Posición |
| `maxSnackbars` | `number` | `5` | Máximo visible |

### ThemeSwitcher

Toggle de tema. Sin props. Se renderiza como botón autocontenido.

### FiltersDialog

Agrupa múltiples filtros en un dialog.
...
```tsx
const { applyOverride, revertOverride, revertAllOverrides } = useThemeOverride();
```

### useElementScroll

```tsx
const { scrollY, scrollDirection } = useElementScroll(ref);
```

### useEnum

```tsx
const { getArray, getInstance } = useEnum(MyEnum);
```
...
| `ListPattern<T>` | `title`, `columns`, `rows`, `searchParamName`, `onAdd`, `addButtonText`, `filtersNode`, `page`, `pages`, `total`, `isLoading` |

---

## Patrones Comunes

### Página CRUD con CrudProvider
...
### Página de listado con ListPattern
